...
The gas is ideal with a constant ratio of specific heats, $\gamma$, and the process is adiabatic, so $p\,V^\gamma = \text{constant}$ in the barrel.
...
\begin{equation}
    \dot{x} \, \mathrm{d} \dot{x} = \frac{A}{m_\text{p}} \left[p_0 \left(\frac{x_0}{x(t)}\right)^\gamma - p_\text{atm} - p_\text{f}\right] \, \mathrm{d} x,
\end{equation}
which can be integrated from rest ($\dot{x}(0) = 0$) to get the projectile kinetic energy as a function of time:
\begin{equation}
    \frac{1}{2} m_\text{p} \dot{x}^2 = A \left[\frac{p_0 \left(x_0^\gamma\,x^{1-\gamma} - x_0\right)}{1 - \gamma} - (p_\text{atm} + p_\text{f}) (x - x_0)\right].
\end{equation}

In terms of the projectile velocity, the result is
\begin{equation}
    \dot{x} = \sqrt{\frac{2 A}{m_\text{p}} \left[\frac{p_0 \left(x_0^\gamma\,x^{1-\gamma} - x_0\right)}{1 - \gamma} - (p_\text{atm} + p_\text{f}) (x - x_0)\right]}.
\end{equation}

This case was implemented in test\_cva.f90 in the subroutine \texttt{test\_one\_cv}.
