...
For Windows, this is blastersim-\gittag-win.zip.

The \texttt{blastersim} executable on Linux and the \texttt{blastersim.exe} executable on Windows can either be placed in the directory you want to run BlasterSim from or placed on your \texttt{PATH}.

\subsection{Installation from source tarball}
\label{sec:installation from source tarball}

The provided source distribution has no dependencies other than a modern Fortran compiler and a Make program like GNU Make on Linux or \href{https://wiki.qt.io/Jom}{jom} or \href{https://learn.microsoft.com/en-us/cpp/build/reference/nmake-reference?view=msvc-170}{NMAKE} on Windows.
...
On Linux you can type \texttt{make blastersim}.

On Windows you can type (if using jom) \texttt{jom blastersim.exe}

Note that compilation of BlasterSim can be slow.

Once built, a BlasterSim executable can be placed appropriately as discussed in the ``From the binary'' section.
...
To be more specific for GNU Make:
\begin{verbatim}
...
On Windows with jom, you should replace \texttt{make blastersim} with \texttt{jom blastersim.exe}.
...
\draw[<->,thick] (\xbarrelstart, \ylbarrel) -- node[above] {$l_\text{travel}$} (\xbarrelend, \ylbarrel);
