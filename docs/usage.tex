...
% TODO: Pneumatic figure like \figref{springer time zero}
% TODO: Pneumatic figure like \figref{springer control volumes}
% TODO: Adjust text below for parallelism with springer mode section
The dead volume is contained only in the barrel in the pneumatic mode.
...
As shown in \figref{springer time zero}, the dead volume is evenly split between the barrel and plunger tube in the springer mode.
This avoids numerical problems from the plunger volume going to zero as the plunger approaches its minimum volume.
BlasterSim geometrically models the plunger tube and barrel are treated as cylindrical.
...
Loose projectiles with less cross-sectional area and also leaks are not handled.
...
\begin{itemize}
\item For springers, a smaller time step (\texttt{dt}) is often required as the plunger approaches the end of the plunger tube.
Plunger impact is more likely at higher flow rates (larger values of \texttt{d\_e}).
Adaptive time stepping should fix this problem in the future.
\item Plunger impact with the end of the plunger tube is not handled yet as discussed in \secref{plunger-impact}.
