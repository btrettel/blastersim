...
\xplungerheadunprimed = \lplungertube - \lplungerhead - \plungermin;
\yxz = \ycenter - (\dplunger + \dbarrel)/4;
\xdbarrel  = \xbarrelend - 1;
\xdplunger = \xplungerheadend + 0.75;
\xmin = \lplungertube - \plungermin;
\xmintext = \lplungertube - \plungermin/2;
...
\draw[thick,dashed] (\xmin, 0) -- (\xmin, \dplunger);
\node[draw=white, rotate=90] at (\xmintext, \ycenter) {$V_\text{dead}/2$};

\fill[fill=gray,draw=black,thick] (\xproj, \ylowbarrel) rectangle ++(\lproj, \dbarrel) node[pos=0.5] {\small\color{white}projectile};

\fill[fill=gray,draw=black,thick] (\xplungerheadstart, 0) rectangle ++(\lplungerhead, \dplunger) node[pos=0.5, rotate=90] {\small\color{white}plunger};
...
\node[draw=white] at (\xdead, \ycenter) {$V_\text{dead}/2$}; % `draw=white` added to make the text fully visible in LaTeXML
...
\end{tikzpicture}
\caption{Model spring blaster at arbitrary time as represented in BlasterSim.\label{fig:springer arbitrary time}}
\end{figure}
...
\draw[<->,thick] (\lfullspring, \halfyfullspringabove) -- node[above, pos=0.4] {$\Delta_\text{pre}$} (\xplungerheadunprimed, \halfyfullspringabove);
\draw[thick,dashed] (\xplungerheadunprimed, \yfullspringabove) -- (\xplungerheadunprimed, 0);

\draw[<->,thick] (\xmin, \ymin) -- node[anchor=west, rotate=90] {$y_\text{min}$} (\lplungertube, \ymin);
\end{tikzpicture}
\caption{Unprimed ($y = y_\text{min}$) model spring blaster as represented in BlasterSim to show spring precompression.\label{fig:springer unprimed}}
\end{figure}
