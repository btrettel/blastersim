...
\begin{align}
    \dv{x_i}{t}       &= \dot{x}_i \\
    \dv{\dot{x}_i}{t} &= \frac{A_i}{m_{\text{eff},i}} \left(p_i - p_{\text{mirror},i} - p_{\text{f},i}\right) - \frac{k_i}{m_{\text{eff},i}} \left(x_i + \Delta_{\text{pre},i}\right) \label{eq:Newton's second law}
\end{align}
...
If a projectile/plunger p has both static and dynamic friction, the friction pressure $p_{\text{f},i}$ could be modeled as a piecewise function:
\begin{equation}
    p_{\text{f},i}(\dot{x}_i, p_{\text{fe},i}) =
    \begin{cases} 
        \sgn(p_{\text{fe},i}) \cdot \min(p_{\text{fs},i}, |p_{\text{fe},i}|), & \text{if}~\dot{x}_i = 0,     \\
        \sgn(\dot{x}_i) \cdot p_{\text{fd},i},                                & \text{if}~\dot{x}_i \neq 0,
    \end{cases}
\end{equation}
where $p_{\text{fe},i}$ is the friction pressure needed to equilibrate the forces on the projectile (where $\dv{\dot{x}_i}{t} = 0$; see~\eqref{Newton's second law}).
...
Also, the use of strict floating point equality in the first branch can lead to numerical reproducibility issues.
BlasterSim uses a smoothed version of this model which avoids these problems.

% TODO
% TODO: Note that BlasterSim's model is not entirely smooth as the direction changing will have a discontinuity. However, that's physically unavoidable, and should be of no consequence as even if a time step was perfectly placed at zero velocity (where the direction is undefined), the static friction model would take over and the direction of the equilibrium pressure will be used instead.
