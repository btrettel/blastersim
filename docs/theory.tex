...
\end{equation}
where $v_\text{s}$ is a small scale over which the $\tanh$ function changes from -1 to 1.

Consequently, the function implemented in cva.f90 as \texttt{p\_f} is
\begin{equation}
    p_{\text{f},i} =
    p_{\text{f0},i}
    + \left[p_{\text{fd},i} - \tanh\left(\frac{\dot{x}_i}{v_\text{s}}\right) \cdot p_{\text{f0},i}\right] \cdot \tanh\left(\frac{\dot{x}_i}{v_\text{s}}\right).
\end{equation}
