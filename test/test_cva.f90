...
subroutine test_m_total(tests)
    ...
    call cv%m_k(2)%v%init_const(5.0_WP, 0)
...
subroutine test_p_cv_ideal(tests)
    ...
    allocate(cv%m_k(1))
    call cv%m_k(1)%v%init_const(RHO_ATM, 0)
    cv%i_cv_mirror = 2
...
subroutine test_r_cv(tests)
    ...
    allocate(cv%m_k(4))
    call cv%m_k(1)%v%init_const(chi(1)*cv%gas(1)%mm, 0)
    call cv%m_k(2)%v%init_const(chi(2)*cv%gas(2)%mm, 0)
    call cv%m_k(3)%v%init_const(chi(3)*cv%gas(3)%mm, 0)
    call cv%m_k(4)%v%init_const(chi(4)*cv%gas(4)%mm, 0)
    
    y(1) = cv%m_k(1) / (cv%m_k(1) + cv%m_k(2) + cv%m_k(3) + cv%m_k(4))
    y(2) = cv%m_k(2) / (cv%m_k(1) + cv%m_k(2) + cv%m_k(3) + cv%m_k(4))
    y(3) = cv%m_k(3) / (cv%m_k(1) + cv%m_k(2) + cv%m_k(3) + cv%m_k(4))
...
subroutine test_p_c(tests)
    ...
    allocate(cv%m_k(2))
    call cv%m_k(1)%v%init_const(m(1), 0)
    call cv%m_k(2)%v%init_const(m(2), 0)
...
subroutine test_set_normal_2(tests)
    ...
    call tests%real_eq(cv%m_k(1)%v%v, m(1), "test_set_normal_2, cv%m_k(1)", abs_tol=1.0e-1_WP)
    call tests%real_eq(cv%m_k(2)%v%v, m(2), "test_set_normal_2, cv%m_k(2)", abs_tol=1.0e-1_WP)
    m_total = cv%m_total()
...
subroutine test_rates(tests)
    use gasdata, only: DRY_AIR
    use cva, only: cv_system_type, d_x_d_t, d_x_dot_d_t, d_m_k_d_t, d_e_g_d_t, d_e_f_d_t
    ...
    type(si_energy_flow_rate) :: h_dots(2, 2), d_e_g_d_t_, d_e_f_d_t_
    ...
    d_e_g_d_t_ = d_e_g_d_t(sys%cv(2), h_dots, 1)
    call tests%real_eq(d_e_g_d_t_%v%v, -p%v%v*csa%v%v*x_dot%v%v + 2.0e5_WP, "d_e_g_d_t(1)")
    
    d_e_g_d_t_ = d_e_g_d_t(sys%cv(2), h_dots, 2)
    call tests%real_eq(d_e_g_d_t_%v%v, -p%v%v*csa%v%v*x_dot%v%v - 2.0e5_WP, "d_e_g_d_t(2)")
...
subroutine test_conservation_2(tests)
    ...
    m_dry_air_start = sys_start%cv(1)%m_k(1) + sys_start%cv(2)%m_k(1)
    m_dry_air_end   = sys_end%cv(1)%m_k(1)   + sys_end%cv(2)%m_k(1)
    m_h2o_start     = sys_start%cv(1)%m_k(2) + sys_start%cv(2)%m_k(2)
    m_h2o_end       = sys_end%cv(1)%m_k(2)   + sys_end%cv(2)%m_k(2)
    
    call tests%real_eq(sys_start%cv(1)%m_k(2)%v%v, 0.0_WP, "test_conservation_2, CV 1 at start has no H2O")
    call tests%real_eq(sys_start%cv(2)%m_k(1)%v%v, 0.0_WP, "test_conservation_2, CV 2 at start has no DRY_AIR")
...
subroutine test_check_sys(tests)
    ...
    sys%cv(1)%peak_x_dot_stop = .false.
    ...
    sys%cv(2)%peak_x_dot_stop = .false.
    call sys%cv(2)%x_min%v%init_const(0.0_WP, n_d)
    
    sys_start = sys
    
    call check_sys(config, sys, sys_start, sys_start, t, status)
...
subroutine exact_x_dot_de(n, ne, ne_d)
    ...
    call sys_start%cv(2)%set(x_0, x_dot, y, p_0, temp_atm, "barrel", csa, 1.0_WP/m_p, p_fs, p_fd, k, &
                                delta_pre, [DRY_AIR], 1, isentropic_filling=.true., p_atm=p_atm, const_friction=.true.)
    
    call assert(sys_start%cv(2)%const_friction, "test_single_cv_exact, cv%const_friction")
...
subroutine test_single_cv_exact(tests)
    ...
    ! Also tests that `cv%const_friction = .true.` works.
